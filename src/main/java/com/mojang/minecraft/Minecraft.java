...
condition|)
block|{
comment|// enforce block deletion delay (survival)
return|return;
block|}
if|if
condition|(
...
condition|)
block|{
comment|// Trigger the hand-waving animation on left-click (?)
name|renderer
operator|.
name|heldBlock
operator|.
...
condition|)
block|{
comment|// There is a block in hand
name|renderer
operator|.
name|heldBlock
operator|.
...
condition|)
block|{
comment|// No block in hand (possible on survival mode)
if|if
condition|(
name|button
operator|==
...
condition|)
block|{
comment|// Ignore clicking on air/water/lava or outside the hitbox of a block (?)
return|return;
block|}
if|if
condition|(
...
block|{
comment|// try parse applet coz sessiondata is set
if|if
condition|(
name|isApplet
condition|)
...
argument_list|)
expr_stmt|;
comment|// LevelIO.save(level, (new FileOutputStream(new
comment|// File(mcDir, "levelc.dat"))));
block|}
else|else
block|{
operator|new
