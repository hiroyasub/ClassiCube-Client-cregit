begin_unit|revision:1.0.0;language:Java;cregit-version:0.0.1
begin_package
package|package
name|com
operator|.
name|mojang
operator|.
name|minecraft
package|;
end_package

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|Serializable
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|ArrayList
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|level
operator|.
name|BlockMap
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|level
operator|.
name|Level
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|level
operator|.
name|liquid
operator|.
name|LiquidType
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|level
operator|.
name|tile
operator|.
name|Block
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|net
operator|.
name|PositionUpdate
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|physics
operator|.
name|AABB
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|player
operator|.
name|Player
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|render
operator|.
name|TextureManager
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|minecraft
operator|.
name|sound
operator|.
name|StepSound
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|util
operator|.
name|ColorCache
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|util
operator|.
name|MathHelper
import|;
end_import

begin_import
import|import
name|com
operator|.
name|mojang
operator|.
name|util
operator|.
name|Vec3D
import|;
end_import

begin_class
specifier|public
specifier|abstract
class|class
name|Entity
implements|implements
name|Serializable
block|{
specifier|public
specifier|static
specifier|final
name|long
name|serialVersionUID
init|=
literal|0L
decl_stmt|;
specifier|public
name|Level
name|level
decl_stmt|;
specifier|public
name|float
name|xo
decl_stmt|;
specifier|public
name|float
name|yo
decl_stmt|;
specifier|public
name|float
name|zo
decl_stmt|;
specifier|public
name|float
name|x
decl_stmt|;
specifier|public
name|float
name|y
decl_stmt|;
specifier|public
name|float
name|z
decl_stmt|;
specifier|public
name|float
name|xd
decl_stmt|;
specifier|public
name|float
name|yd
decl_stmt|;
specifier|public
name|float
name|zd
decl_stmt|;
specifier|public
name|float
name|yRot
decl_stmt|;
specifier|public
name|float
name|xRot
decl_stmt|;
specifier|public
name|float
name|yRotO
decl_stmt|;
specifier|public
name|float
name|xRotO
decl_stmt|;
comment|/**      * The bounding box of this Entity.      */
specifier|public
name|AABB
name|boundingBox
decl_stmt|;
specifier|public
name|boolean
name|onGround
init|=
literal|false
decl_stmt|;
specifier|public
name|boolean
name|horizontalCollision
init|=
literal|false
decl_stmt|;
specifier|public
name|boolean
name|collision
init|=
literal|false
decl_stmt|;
specifier|public
name|boolean
name|slide
init|=
literal|true
decl_stmt|;
specifier|public
name|boolean
name|removed
init|=
literal|false
decl_stmt|;
specifier|public
name|float
name|heightOffset
init|=
literal|0F
decl_stmt|;
specifier|public
name|float
name|bbWidth
init|=
literal|0.6F
decl_stmt|;
specifier|public
name|float
name|bbHeight
init|=
literal|1.8F
decl_stmt|;
specifier|public
name|float
name|walkDistO
init|=
literal|0F
decl_stmt|;
specifier|public
name|float
name|walkDist
init|=
literal|0F
decl_stmt|;
specifier|public
name|boolean
name|makeStepSound
init|=
literal|true
decl_stmt|;
specifier|public
name|float
name|fallDistance
init|=
literal|0F
decl_stmt|;
specifier|public
name|BlockMap
name|blockMap
decl_stmt|;
specifier|public
name|float
name|xOld
decl_stmt|;
specifier|public
name|float
name|yOld
decl_stmt|;
specifier|public
name|float
name|zOld
decl_stmt|;
specifier|public
name|int
name|textureId
init|=
literal|0
decl_stmt|;
specifier|public
name|float
name|ySlideOffset
init|=
literal|0F
decl_stmt|;
specifier|public
name|float
name|footSize
init|=
literal|0F
decl_stmt|;
specifier|public
name|boolean
name|noPhysics
init|=
literal|false
decl_stmt|;
specifier|public
name|float
name|pushthrough
init|=
literal|0F
decl_stmt|;
specifier|public
name|boolean
name|hovered
init|=
literal|false
decl_stmt|;
specifier|public
name|boolean
name|flyingMode
init|=
literal|false
decl_stmt|;
specifier|public
name|float
name|prevDistanceWalkedModified
decl_stmt|;
specifier|public
name|float
name|distanceWalkedModified
decl_stmt|;
specifier|public
name|float
name|distanceWalkedOnStepModified
decl_stmt|;
specifier|private
name|int
name|nextStep
init|=
literal|1
decl_stmt|;
specifier|private
name|int
name|nextStepDistance
decl_stmt|;
specifier|public
name|Entity
parameter_list|(
name|Level
name|entityLevel
parameter_list|)
block|{
name|level
operator|=
name|entityLevel
expr_stmt|;
name|this
operator|.
name|setPos
argument_list|(
literal|0F
argument_list|,
literal|0F
argument_list|,
literal|0F
argument_list|)
expr_stmt|;
block|}
specifier|public
name|void
name|awardKillScore
parameter_list|(
name|Entity
name|var1
parameter_list|,
name|int
name|var2
parameter_list|)
block|{
block|}
specifier|protected
name|void
name|causeFallDamage
parameter_list|(
name|float
name|var1
parameter_list|)
block|{
block|}
comment|/**      * Calculates the distance from this entity to the specified entity.      *      * @param otherEntity Entity to calculate the distance to.      * @return The distance between the two entities.      */
specifier|public
name|float
name|distanceTo
parameter_list|(
name|Entity
name|otherEntity
parameter_list|)
block|{
return|return
name|distanceTo
argument_list|(
name|otherEntity
operator|.
name|x
argument_list|,
name|otherEntity
operator|.
name|y
argument_list|,
name|otherEntity
operator|.
name|z
argument_list|)
return|;
block|}
comment|/**      * Calculates the distance from this entity to the specified position.      *      * @param posX X-Coordinate of the position to calculate the distance to.      * @param posY Y-Coordinate of the position to calculate the distance to.      * @param posZ Z-Coordinate of the position to calculate the distance to.      * @return The distance between the entity and the position.      */
specifier|public
name|float
name|distanceTo
parameter_list|(
name|float
name|posX
parameter_list|,
name|float
name|posY
parameter_list|,
name|float
name|posZ
parameter_list|)
block|{
comment|// Euclidean distance
name|float
name|dx
init|=
name|x
operator|-
name|posX
decl_stmt|;
name|float
name|dy
init|=
name|y
operator|-
name|posY
decl_stmt|;
name|float
name|dz
init|=
name|z
operator|-
name|posZ
decl_stmt|;
return|return
name|MathHelper
operator|.
name|sqrt
argument_list|(
operator|(
name|dx
operator|*
name|dx
operator|)
operator|+
operator|(
name|dy
operator|*
name|dy
operator|)
operator|+
operator|(
name|dz
operator|*
name|dz
operator|)
argument_list|)
return|;
block|}
comment|/**      * Calculates the distance from this entity to the specified entity squared.      * This is basically calculating distance without using the expensive      * Math.sqrt function. Should only be used for relative distance.      *      * @param otherEntity Entity to calculate the distance to.      * @return The distance between the two entities squared.      */
specifier|public
name|float
name|distanceToSqr
parameter_list|(
name|Entity
name|otherEntity
parameter_list|)
block|{
name|float
name|dx
init|=
name|x
operator|-
name|otherEntity
operator|.
name|x
decl_stmt|;
name|float
name|dy
init|=
name|y
operator|-
name|otherEntity
operator|.
name|y
decl_stmt|;
name|float
name|dz
init|=
name|z
operator|-
name|otherEntity
operator|.
name|z
decl_stmt|;
return|return
operator|(
name|dx
operator|*
name|dx
operator|)
operator|+
operator|(
name|dy
operator|*
name|dy
operator|)
operator|+
operator|(
name|dz
operator|*
name|dz
operator|)
return|;
block|}
comment|/**      * Gets the brightness of this entity      *      * @return Brightness of the entity.      */
specifier|public
name|float
name|getBrightness
parameter_list|()
block|{
name|int
name|posX
init|=
operator|(
name|int
operator|)
name|x
decl_stmt|;
name|int
name|posY
init|=
operator|(
name|int
operator|)
operator|(
name|y
operator|+
name|heightOffset
operator|/
literal|2F
operator|-
literal|0.5F
operator|)
decl_stmt|;
name|int
name|posZ
init|=
operator|(
name|int
operator|)
name|z
decl_stmt|;
return|return
name|level
operator|.
name|getBrightness
argument_list|(
name|posX
argument_list|,
name|posY
argument_list|,
name|posZ
argument_list|)
return|;
block|}
comment|/**      * Gets the brightness color of this entity.      *      * @return ColorCache containing brightness color information.      */
specifier|public
name|ColorCache
name|getBrightnessColor
parameter_list|()
block|{
name|int
name|posX
init|=
operator|(
name|int
operator|)
name|x
decl_stmt|;
name|int
name|posY
init|=
operator|(
name|int
operator|)
operator|(
name|y
operator|+
name|heightOffset
operator|/
literal|2F
operator|-
literal|0.5F
operator|)
decl_stmt|;
name|int
name|posZ
init|=
operator|(
name|int
operator|)
name|z
decl_stmt|;
return|return
name|level
operator|.
name|getBrightnessColor
argument_list|(
name|posX
argument_list|,
name|posY
argument_list|,
name|posZ
argument_list|)
return|;
block|}
comment|/**      * Gets the texture ID of this entity.      *      * @return Entity's Texture ID.      */
specifier|public
name|int
name|getTexture
parameter_list|()
block|{
return|return
name|textureId
return|;
block|}
specifier|public
name|void
name|hurt
parameter_list|(
name|Entity
name|entity
parameter_list|,
name|int
name|hurtAmount
parameter_list|)
block|{
block|}
comment|// TODO var1 var2
specifier|public
name|void
name|interpolateTurn
parameter_list|(
name|float
name|var1
parameter_list|,
name|float
name|var2
parameter_list|)
block|{
name|yRot
operator|=
operator|(
name|float
operator|)
operator|(
name|yRot
operator|+
name|var1
operator|*
literal|0.15D
operator|)
expr_stmt|;
name|xRot
operator|=
operator|(
name|float
operator|)
operator|(
name|xRot
operator|-
name|var2
operator|*
literal|0.15D
operator|)
expr_stmt|;
if|if
condition|(
name|xRot
operator|<
operator|-
literal|90F
condition|)
block|{
name|xRot
operator|=
operator|-
literal|90F
expr_stmt|;
block|}
if|if
condition|(
name|xRot
operator|>
literal|90F
condition|)
block|{
name|xRot
operator|=
literal|90F
expr_stmt|;
block|}
block|}
specifier|public
name|boolean
name|intersects
parameter_list|(
name|float
name|x0
parameter_list|,
name|float
name|y0
parameter_list|,
name|float
name|z0
parameter_list|,
name|float
name|x1
parameter_list|,
name|float
name|y1
parameter_list|,
name|float
name|z1
parameter_list|)
block|{
return|return
name|boundingBox
operator|.
name|intersects
argument_list|(
name|x0
argument_list|,
name|y0
argument_list|,
name|z0
argument_list|,
name|x1
argument_list|,
name|y1
argument_list|,
name|z1
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|isCreativeModeAllowed
parameter_list|()
block|{
return|return
literal|false
return|;
block|}
specifier|public
name|boolean
name|isFree
parameter_list|(
name|float
name|x
parameter_list|,
name|float
name|y
parameter_list|,
name|float
name|z
parameter_list|)
block|{
name|AABB
name|bounds
init|=
name|boundingBox
operator|.
name|cloneMove
argument_list|(
name|x
argument_list|,
name|y
argument_list|,
name|z
argument_list|)
decl_stmt|;
return|return
name|level
operator|.
name|getCubes
argument_list|(
name|bounds
argument_list|)
operator|.
name|size
argument_list|()
operator|<=
literal|0
operator|&&
operator|!
name|level
operator|.
name|containsAnyLiquid
argument_list|(
name|bounds
argument_list|)
return|;
block|}
comment|// TODO - growAmount may not be an accurate interpretation
specifier|public
name|boolean
name|isFree
parameter_list|(
name|float
name|x
parameter_list|,
name|float
name|y
parameter_list|,
name|float
name|z
parameter_list|,
name|float
name|growAmount
parameter_list|)
block|{
name|AABB
name|bounds
init|=
name|boundingBox
operator|.
name|grow
argument_list|(
name|growAmount
argument_list|,
name|growAmount
argument_list|,
name|growAmount
argument_list|)
operator|.
name|cloneMove
argument_list|(
name|x
argument_list|,
name|y
argument_list|,
name|z
argument_list|)
decl_stmt|;
return|return
name|level
operator|.
name|getCubes
argument_list|(
name|bounds
argument_list|)
operator|.
name|size
argument_list|()
operator|<=
literal|0
operator|&&
operator|!
name|level
operator|.
name|containsAnyLiquid
argument_list|(
name|bounds
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|isInLava
parameter_list|()
block|{
return|return
name|level
operator|.
name|containsLiquid
argument_list|(
name|boundingBox
operator|.
name|grow
argument_list|(
literal|0F
argument_list|,
operator|-
literal|0.4F
argument_list|,
literal|0F
argument_list|)
argument_list|,
name|LiquidType
operator|.
name|lava
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|isInOrOnRope
parameter_list|()
block|{
return|return
name|level
operator|.
name|containsBlock
argument_list|(
name|boundingBox
operator|.
name|grow
argument_list|(
operator|-
literal|0.5F
argument_list|,
literal|0F
argument_list|,
operator|-
literal|0.5F
argument_list|)
argument_list|,
name|Block
operator|.
name|ROPE
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|isInWater
parameter_list|()
block|{
return|return
name|level
operator|.
name|containsLiquid
argument_list|(
name|boundingBox
operator|.
name|grow
argument_list|(
literal|0F
argument_list|,
operator|-
literal|0.4F
argument_list|,
literal|0F
argument_list|)
argument_list|,
name|LiquidType
operator|.
name|water
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|isLit
parameter_list|()
block|{
name|int
name|iX
init|=
operator|(
name|int
operator|)
name|x
decl_stmt|;
name|int
name|iY
init|=
operator|(
name|int
operator|)
name|y
decl_stmt|;
name|int
name|iZ
init|=
operator|(
name|int
operator|)
name|z
decl_stmt|;
return|return
name|level
operator|.
name|isLit
argument_list|(
name|iX
argument_list|,
name|iY
argument_list|,
name|iZ
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|isPickable
parameter_list|()
block|{
return|return
literal|false
return|;
block|}
specifier|public
name|boolean
name|isPushable
parameter_list|()
block|{
return|return
literal|false
return|;
block|}
specifier|public
name|boolean
name|isShootable
parameter_list|()
block|{
return|return
literal|false
return|;
block|}
specifier|public
name|boolean
name|isUnderWater
parameter_list|()
block|{
name|int
name|textureID
decl_stmt|;
return|return
operator|(
name|textureID
operator|=
name|level
operator|.
name|getTile
argument_list|(
operator|(
name|int
operator|)
name|x
argument_list|,
operator|(
name|int
operator|)
operator|(
name|y
operator|+
literal|0.12F
operator|)
argument_list|,
operator|(
name|int
operator|)
name|z
argument_list|)
operator|)
operator|!=
literal|0
operator|&&
name|Block
operator|.
name|blocks
index|[
name|textureID
index|]
operator|.
name|getLiquidType
argument_list|()
operator|.
name|equals
argument_list|(
name|LiquidType
operator|.
name|water
argument_list|)
return|;
block|}
specifier|public
name|void
name|move
parameter_list|(
name|float
name|xMove
parameter_list|,
name|float
name|yMove
parameter_list|,
name|float
name|zMove
parameter_list|)
block|{
if|if
condition|(
name|noPhysics
condition|)
block|{
name|boundingBox
operator|.
name|move
argument_list|(
name|xMove
argument_list|,
name|yMove
argument_list|,
name|zMove
argument_list|)
expr_stmt|;
name|x
operator|=
operator|(
name|boundingBox
operator|.
name|maxX
operator|+
name|boundingBox
operator|.
name|minX
operator|)
operator|/
literal|2F
expr_stmt|;
name|y
operator|=
name|boundingBox
operator|.
name|maxY
operator|+
name|heightOffset
operator|-
name|ySlideOffset
expr_stmt|;
name|z
operator|=
operator|(
name|boundingBox
operator|.
name|maxZ
operator|+
name|boundingBox
operator|.
name|minZ
operator|)
operator|/
literal|2F
expr_stmt|;
block|}
else|else
block|{
name|float
name|var4
init|=
name|x
decl_stmt|;
name|float
name|var5
init|=
name|z
decl_stmt|;
name|float
name|var6
init|=
name|xMove
decl_stmt|;
name|float
name|var7
init|=
name|yMove
decl_stmt|;
name|float
name|var8
init|=
name|zMove
decl_stmt|;
name|AABB
name|bbCopy
init|=
name|boundingBox
operator|.
name|copy
argument_list|()
decl_stmt|;
name|ArrayList
argument_list|<
name|AABB
argument_list|>
name|cubes
init|=
name|level
operator|.
name|getCubes
argument_list|(
name|boundingBox
operator|.
name|expand
argument_list|(
name|xMove
argument_list|,
name|yMove
argument_list|,
name|zMove
argument_list|)
argument_list|)
decl_stmt|;
for|for
control|(
name|AABB
name|cube5
range|:
name|cubes
control|)
block|{
name|yMove
operator|=
name|cube5
operator|.
name|clipYCollide
argument_list|(
name|boundingBox
argument_list|,
name|yMove
argument_list|)
expr_stmt|;
block|}
name|boundingBox
operator|.
name|move
argument_list|(
literal|0F
argument_list|,
name|yMove
argument_list|,
literal|0F
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|slide
operator|&&
name|var7
operator|!=
name|yMove
condition|)
block|{
name|zMove
operator|=
literal|0F
expr_stmt|;
name|yMove
operator|=
literal|0F
expr_stmt|;
name|xMove
operator|=
literal|0F
expr_stmt|;
block|}
name|boolean
name|var16
init|=
name|onGround
operator|||
name|var7
operator|!=
name|yMove
operator|&&
name|var7
operator|<
literal|0F
decl_stmt|;
for|for
control|(
name|AABB
name|cube4
range|:
name|cubes
control|)
block|{
name|xMove
operator|=
name|cube4
operator|.
name|clipXCollide
argument_list|(
name|boundingBox
argument_list|,
name|xMove
argument_list|)
expr_stmt|;
block|}
name|boundingBox
operator|.
name|move
argument_list|(
name|xMove
argument_list|,
literal|0F
argument_list|,
literal|0F
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|slide
operator|&&
name|var6
operator|!=
name|xMove
condition|)
block|{
name|zMove
operator|=
literal|0F
expr_stmt|;
name|yMove
operator|=
literal|0F
expr_stmt|;
name|xMove
operator|=
literal|0F
expr_stmt|;
block|}
for|for
control|(
name|AABB
name|cube3
range|:
name|cubes
control|)
block|{
name|zMove
operator|=
name|cube3
operator|.
name|clipZCollide
argument_list|(
name|boundingBox
argument_list|,
name|zMove
argument_list|)
expr_stmt|;
block|}
name|boundingBox
operator|.
name|move
argument_list|(
literal|0F
argument_list|,
literal|0F
argument_list|,
name|zMove
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|slide
operator|&&
name|var8
operator|!=
name|zMove
condition|)
block|{
name|zMove
operator|=
literal|0F
expr_stmt|;
name|yMove
operator|=
literal|0F
expr_stmt|;
name|xMove
operator|=
literal|0F
expr_stmt|;
block|}
name|float
name|var17
decl_stmt|;
name|float
name|var18
decl_stmt|;
if|if
condition|(
name|footSize
operator|>
literal|0F
operator|&&
name|var16
operator|&&
name|ySlideOffset
operator|<
literal|0.05F
operator|&&
operator|(
name|var6
operator|!=
name|xMove
operator|||
name|var8
operator|!=
name|zMove
operator|)
condition|)
block|{
name|var18
operator|=
name|xMove
expr_stmt|;
name|var17
operator|=
name|yMove
expr_stmt|;
name|float
name|var13
init|=
name|zMove
decl_stmt|;
name|xMove
operator|=
name|var6
expr_stmt|;
name|yMove
operator|=
name|footSize
expr_stmt|;
name|zMove
operator|=
name|var8
expr_stmt|;
name|AABB
name|var14
init|=
name|boundingBox
operator|.
name|copy
argument_list|()
decl_stmt|;
name|boundingBox
operator|=
name|bbCopy
operator|.
name|copy
argument_list|()
expr_stmt|;
name|cubes
operator|=
name|level
operator|.
name|getCubes
argument_list|(
name|boundingBox
operator|.
name|expand
argument_list|(
name|var6
argument_list|,
name|yMove
argument_list|,
name|var8
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|AABB
name|cube2
range|:
name|cubes
control|)
block|{
name|yMove
operator|=
name|cube2
operator|.
name|clipYCollide
argument_list|(
name|boundingBox
argument_list|,
name|yMove
argument_list|)
expr_stmt|;
block|}
name|boundingBox
operator|.
name|move
argument_list|(
literal|0F
argument_list|,
name|yMove
argument_list|,
literal|0F
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|slide
operator|&&
name|var7
operator|!=
name|yMove
condition|)
block|{
name|zMove
operator|=
literal|0F
expr_stmt|;
name|yMove
operator|=
literal|0F
expr_stmt|;
name|xMove
operator|=
literal|0F
expr_stmt|;
block|}
for|for
control|(
name|AABB
name|cube1
range|:
name|cubes
control|)
block|{
name|xMove
operator|=
name|cube1
operator|.
name|clipXCollide
argument_list|(
name|boundingBox
argument_list|,
name|xMove
argument_list|)
expr_stmt|;
block|}
name|boundingBox
operator|.
name|move
argument_list|(
name|xMove
argument_list|,
literal|0F
argument_list|,
literal|0F
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|slide
operator|&&
name|var6
operator|!=
name|xMove
condition|)
block|{
name|zMove
operator|=
literal|0F
expr_stmt|;
name|yMove
operator|=
literal|0F
expr_stmt|;
name|xMove
operator|=
literal|0F
expr_stmt|;
block|}
for|for
control|(
name|AABB
name|cube
range|:
name|cubes
control|)
block|{
name|zMove
operator|=
name|cube
operator|.
name|clipZCollide
argument_list|(
name|boundingBox
argument_list|,
name|zMove
argument_list|)
expr_stmt|;
block|}
name|boundingBox
operator|.
name|move
argument_list|(
literal|0F
argument_list|,
literal|0F
argument_list|,
name|zMove
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|slide
operator|&&
name|var8
operator|!=
name|zMove
condition|)
block|{
name|zMove
operator|=
literal|0F
expr_stmt|;
name|yMove
operator|=
literal|0F
expr_stmt|;
name|xMove
operator|=
literal|0F
expr_stmt|;
block|}
if|if
condition|(
name|var18
operator|*
name|var18
operator|+
name|var13
operator|*
name|var13
operator|>=
name|xMove
operator|*
name|xMove
operator|+
name|zMove
operator|*
name|zMove
condition|)
block|{
name|xMove
operator|=
name|var18
expr_stmt|;
name|yMove
operator|=
name|var17
expr_stmt|;
name|zMove
operator|=
name|var13
expr_stmt|;
name|boundingBox
operator|=
name|var14
operator|.
name|copy
argument_list|()
expr_stmt|;
block|}
else|else
block|{
name|ySlideOffset
operator|=
operator|(
name|float
operator|)
operator|(
name|ySlideOffset
operator|+
literal|0.5D
operator|)
expr_stmt|;
block|}
block|}
name|horizontalCollision
operator|=
name|var6
operator|!=
name|xMove
operator|||
name|var8
operator|!=
name|zMove
expr_stmt|;
name|onGround
operator|=
name|var7
operator|!=
name|yMove
operator|&&
name|var7
operator|<
literal|0F
expr_stmt|;
name|collision
operator|=
name|horizontalCollision
operator|||
name|var7
operator|!=
name|yMove
expr_stmt|;
if|if
condition|(
name|onGround
condition|)
block|{
if|if
condition|(
name|fallDistance
operator|>
literal|0F
condition|)
block|{
name|causeFallDamage
argument_list|(
name|fallDistance
operator|/
literal|2
argument_list|)
expr_stmt|;
name|fallDistance
operator|=
literal|0F
expr_stmt|;
block|}
block|}
if|else if
condition|(
name|yMove
operator|<
literal|0F
condition|)
block|{
name|fallDistance
operator|-=
name|yMove
expr_stmt|;
block|}
if|if
condition|(
name|var6
operator|!=
name|xMove
condition|)
block|{
name|xd
operator|=
literal|0F
expr_stmt|;
block|}
if|if
condition|(
name|var7
operator|!=
name|yMove
condition|)
block|{
name|yd
operator|=
literal|0F
expr_stmt|;
block|}
if|if
condition|(
name|var8
operator|!=
name|zMove
condition|)
block|{
name|zd
operator|=
literal|0F
expr_stmt|;
block|}
name|x
operator|=
operator|(
name|boundingBox
operator|.
name|maxX
operator|+
name|boundingBox
operator|.
name|minX
operator|)
operator|/
literal|2F
expr_stmt|;
name|y
operator|=
name|boundingBox
operator|.
name|maxY
operator|+
name|heightOffset
operator|-
name|ySlideOffset
expr_stmt|;
name|z
operator|=
operator|(
name|boundingBox
operator|.
name|maxZ
operator|+
name|boundingBox
operator|.
name|minZ
operator|)
operator|/
literal|2F
expr_stmt|;
name|var18
operator|=
name|x
operator|-
name|var4
expr_stmt|;
name|var17
operator|=
name|z
operator|-
name|var5
expr_stmt|;
name|walkDist
operator|=
operator|(
name|float
operator|)
operator|(
name|walkDist
operator|+
name|MathHelper
operator|.
name|sqrt
argument_list|(
name|var18
operator|*
name|var18
operator|+
name|var17
operator|*
name|var17
argument_list|)
operator|*
literal|0.6D
operator|)
expr_stmt|;
block|}
name|int
name|var39
init|=
operator|(
name|int
operator|)
name|Math
operator|.
name|floor
argument_list|(
name|x
argument_list|)
decl_stmt|;
name|int
name|var30
init|=
operator|(
name|int
operator|)
name|Math
operator|.
name|floor
argument_list|(
name|y
operator|-
literal|0.20000000298023224D
operator|-
name|heightOffset
argument_list|)
decl_stmt|;
name|int
name|var31
init|=
operator|(
name|int
operator|)
name|Math
operator|.
name|floor
argument_list|(
name|z
argument_list|)
decl_stmt|;
name|int
name|var32
init|=
name|level
operator|.
name|getTile
argument_list|(
name|var39
argument_list|,
name|var30
argument_list|,
name|var31
argument_list|)
decl_stmt|;
if|if
condition|(
name|makeStepSound
operator|&&
name|onGround
operator|&&
operator|!
name|noPhysics
condition|)
block|{
if|if
condition|(
name|this
operator|instanceof
name|Player
operator|&&
operator|!
operator|(
operator|(
name|Player
operator|)
name|this
operator|)
operator|.
name|noPhysics
condition|)
block|{
name|distanceWalkedModified
operator|=
operator|(
name|float
operator|)
operator|(
name|distanceWalkedModified
operator|+
name|Math
operator|.
name|sqrt
argument_list|(
name|xMove
operator|*
name|xMove
operator|+
name|zMove
operator|*
name|zMove
argument_list|)
operator|*
literal|0.6D
operator|)
expr_stmt|;
name|distanceWalkedOnStepModified
operator|=
operator|(
name|float
operator|)
operator|(
name|distanceWalkedOnStepModified
operator|+
name|Math
operator|.
name|sqrt
argument_list|(
name|xMove
operator|*
name|xMove
operator|+
name|yMove
operator|*
name|yMove
operator|+
name|zMove
operator|*
name|zMove
argument_list|)
operator|*
literal|0.6D
operator|)
expr_stmt|;
if|if
condition|(
name|distanceWalkedOnStepModified
operator|>
name|nextStepDistance
operator|&&
name|var32
operator|>
literal|0
condition|)
block|{
name|nextStepDistance
operator|=
operator|(
name|int
operator|)
name|distanceWalkedOnStepModified
operator|+
literal|1
expr_stmt|;
if|if
condition|(
name|onGround
condition|)
block|{
name|playStepSound
argument_list|(
name|var32
argument_list|)
expr_stmt|;
block|}
block|}
block|}
block|}
if|if
condition|(
name|walkDist
operator|>
name|nextStep
operator|&&
name|var32
operator|>
literal|0
condition|)
block|{
operator|++
name|nextStep
expr_stmt|;
block|}
name|ySlideOffset
operator|*=
literal|0.4F
expr_stmt|;
block|}
comment|// Moves horizontally in the direction specified by vector [dX,dZ]
comment|// relative to current rotation, by [distance] units.
comment|// Length of vector [dX,dZ] must be at least 0.01 to have effect.
specifier|public
name|void
name|moveRelative
parameter_list|(
name|float
name|dX
parameter_list|,
name|float
name|dZ
parameter_list|,
name|float
name|distance
parameter_list|)
block|{
name|float
name|vecLength
init|=
name|MathHelper
operator|.
name|sqrt
argument_list|(
name|dX
operator|*
name|dX
operator|+
name|dZ
operator|*
name|dZ
argument_list|)
decl_stmt|;
if|if
condition|(
name|vecLength
operator|>=
literal|0.01F
condition|)
block|{
if|if
condition|(
name|vecLength
operator|<
literal|1F
condition|)
block|{
name|vecLength
operator|=
literal|1F
expr_stmt|;
block|}
name|float
name|normalizeAndScale
init|=
name|distance
operator|/
name|vecLength
decl_stmt|;
name|dX
operator|*=
name|normalizeAndScale
expr_stmt|;
name|dZ
operator|*=
name|normalizeAndScale
expr_stmt|;
name|float
name|rotRadians
init|=
name|yRot
operator|*
operator|(
name|float
operator|)
name|Math
operator|.
name|PI
operator|/
literal|180F
decl_stmt|;
name|float
name|sin
init|=
name|MathHelper
operator|.
name|sin
argument_list|(
name|rotRadians
argument_list|)
decl_stmt|;
name|float
name|cos
init|=
name|MathHelper
operator|.
name|cos
argument_list|(
name|rotRadians
argument_list|)
decl_stmt|;
name|xd
operator|+=
name|dX
operator|*
name|cos
operator|-
name|dZ
operator|*
name|sin
expr_stmt|;
name|zd
operator|+=
name|dZ
operator|*
name|cos
operator|+
name|dX
operator|*
name|sin
expr_stmt|;
block|}
block|}
specifier|public
name|void
name|moveTo
parameter_list|(
name|float
name|x
parameter_list|,
name|float
name|y
parameter_list|,
name|float
name|z
parameter_list|,
name|float
name|pitch
parameter_list|,
name|float
name|yaw
parameter_list|)
block|{
name|xo
operator|=
name|this
operator|.
name|x
operator|=
name|x
expr_stmt|;
name|yo
operator|=
name|this
operator|.
name|y
operator|=
name|y
expr_stmt|;
name|zo
operator|=
name|this
operator|.
name|z
operator|=
name|z
expr_stmt|;
name|yRot
operator|=
name|pitch
expr_stmt|;
name|xRot
operator|=
name|yaw
expr_stmt|;
name|this
operator|.
name|setPos
argument_list|(
name|x
argument_list|,
name|y
argument_list|,
name|z
argument_list|)
expr_stmt|;
block|}
specifier|public
name|void
name|playerTouch
parameter_list|(
name|Entity
name|entity
parameter_list|)
block|{
block|}
specifier|public
name|void
name|playSound
parameter_list|(
name|String
name|file
parameter_list|,
name|float
name|volume
parameter_list|,
name|float
name|pitch
parameter_list|)
block|{
name|level
operator|.
name|playSound
argument_list|(
name|file
argument_list|,
name|this
argument_list|,
name|volume
argument_list|,
name|pitch
argument_list|,
literal|false
argument_list|)
expr_stmt|;
block|}
specifier|protected
name|void
name|playStepSound
parameter_list|(
name|int
name|tile
parameter_list|)
block|{
name|StepSound
name|sound
init|=
name|Block
operator|.
name|blocks
index|[
name|tile
index|]
operator|.
name|stepSound
decl_stmt|;
if|if
condition|(
operator|!
name|Block
operator|.
name|blocks
index|[
name|tile
index|]
operator|.
name|isLiquid
argument_list|()
condition|)
block|{
name|playSound
argument_list|(
name|sound
operator|.
name|getStepSound
argument_list|()
argument_list|,
name|sound
operator|.
name|getVolume
argument_list|()
operator|*
literal|0.70F
argument_list|,
name|sound
operator|.
name|getPitch
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
specifier|public
name|void
name|push
parameter_list|(
name|Entity
name|entity
parameter_list|)
block|{
name|float
name|dx
init|=
name|entity
operator|.
name|x
operator|-
name|x
decl_stmt|;
name|float
name|dz
init|=
name|entity
operator|.
name|z
operator|-
name|z
decl_stmt|;
name|float
name|dy
decl_stmt|;
if|if
condition|(
operator|(
name|dy
operator|=
name|dx
operator|*
name|dx
operator|+
name|dz
operator|*
name|dz
operator|)
operator|>=
literal|0.01F
condition|)
block|{
name|dy
operator|=
name|MathHelper
operator|.
name|sqrt
argument_list|(
name|dy
argument_list|)
expr_stmt|;
name|dx
operator|/=
name|dy
expr_stmt|;
name|dz
operator|/=
name|dy
expr_stmt|;
name|dx
operator|/=
name|dy
expr_stmt|;
name|dz
operator|/=
name|dy
expr_stmt|;
name|dx
operator|*=
literal|0.05F
expr_stmt|;
name|dz
operator|*=
literal|0.05F
expr_stmt|;
name|dx
operator|*=
literal|1F
operator|-
name|pushthrough
expr_stmt|;
name|dz
operator|*=
literal|1F
operator|-
name|pushthrough
expr_stmt|;
name|this
operator|.
name|push
argument_list|(
operator|-
name|dx
argument_list|,
literal|0F
argument_list|,
operator|-
name|dz
argument_list|)
expr_stmt|;
name|entity
operator|.
name|push
argument_list|(
name|dx
argument_list|,
literal|0F
argument_list|,
name|dz
argument_list|)
expr_stmt|;
block|}
block|}
specifier|protected
name|void
name|push
parameter_list|(
name|float
name|x
parameter_list|,
name|float
name|y
parameter_list|,
name|float
name|z
parameter_list|)
block|{
name|xd
operator|+=
name|x
expr_stmt|;
name|yd
operator|+=
name|y
expr_stmt|;
name|zd
operator|+=
name|z
expr_stmt|;
block|}
specifier|public
name|void
name|remove
parameter_list|()
block|{
name|removed
operator|=
literal|true
expr_stmt|;
block|}
specifier|public
name|void
name|render
parameter_list|(
name|TextureManager
name|textureManager
parameter_list|,
name|float
name|delta
parameter_list|)
block|{
block|}
specifier|public
name|void
name|renderHover
parameter_list|(
name|TextureManager
name|textureManager
parameter_list|)
block|{
block|}
specifier|public
name|void
name|resetPos
parameter_list|()
block|{
if|if
condition|(
name|level
operator|!=
literal|null
condition|)
block|{
name|float
name|xSpawn
init|=
name|level
operator|.
name|xSpawn
operator|+
literal|0.5F
decl_stmt|;
name|float
name|ySpawn
init|=
name|level
operator|.
name|ySpawn
decl_stmt|;
for|for
control|(
name|double
name|zSpawn
init|=
name|level
operator|.
name|zSpawn
operator|+
literal|0.5F
init|;
name|ySpawn
operator|>
literal|0F
condition|;
operator|++
name|ySpawn
control|)
block|{
name|this
operator|.
name|setPos
argument_list|(
name|xSpawn
argument_list|,
name|ySpawn
argument_list|,
operator|(
name|float
operator|)
name|zSpawn
argument_list|)
expr_stmt|;
if|if
condition|(
name|level
operator|.
name|isInBounds
argument_list|(
operator|(
name|int
operator|)
name|xSpawn
argument_list|,
operator|(
name|int
operator|)
name|ySpawn
argument_list|,
operator|(
name|int
operator|)
name|zSpawn
argument_list|)
condition|)
block|{
if|if
condition|(
name|level
operator|.
name|getCubes
argument_list|(
name|boundingBox
argument_list|)
operator|.
name|isEmpty
argument_list|()
condition|)
block|{
break|break;
block|}
block|}
else|else
block|{
name|ySpawn
operator|=
name|level
operator|.
name|ySpawn
expr_stmt|;
break|break;
block|}
block|}
name|yRot
operator|=
name|level
operator|.
name|rotSpawn
expr_stmt|;
block|}
block|}
specifier|public
name|void
name|setLevel
parameter_list|(
name|Level
name|entityLevel
parameter_list|)
block|{
name|level
operator|=
name|entityLevel
expr_stmt|;
block|}
specifier|public
name|void
name|setPos
parameter_list|(
name|float
name|x
parameter_list|,
name|float
name|y
parameter_list|,
name|float
name|z
parameter_list|)
block|{
name|this
operator|.
name|x
operator|=
name|x
expr_stmt|;
name|this
operator|.
name|y
operator|=
name|y
expr_stmt|;
name|this
operator|.
name|z
operator|=
name|z
expr_stmt|;
name|float
name|middleWidth
init|=
name|bbWidth
operator|/
literal|2F
decl_stmt|;
name|float
name|middleHeight
init|=
name|bbHeight
operator|/
literal|2F
decl_stmt|;
name|boundingBox
operator|=
operator|new
name|AABB
argument_list|(
name|x
operator|-
name|middleWidth
argument_list|,
name|y
operator|-
name|middleHeight
argument_list|,
name|z
operator|-
name|middleWidth
argument_list|,
name|x
operator|+
name|middleWidth
argument_list|,
name|y
operator|+
name|middleHeight
argument_list|,
name|z
operator|+
name|middleWidth
argument_list|)
expr_stmt|;
block|}
specifier|public
name|void
name|setPos
parameter_list|(
name|PositionUpdate
name|update
parameter_list|)
block|{
if|if
condition|(
name|update
operator|.
name|position
condition|)
block|{
name|this
operator|.
name|setPos
argument_list|(
name|update
operator|.
name|x
argument_list|,
name|update
operator|.
name|y
argument_list|,
name|update
operator|.
name|z
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|this
operator|.
name|setPos
argument_list|(
name|x
argument_list|,
name|y
argument_list|,
name|z
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|update
operator|.
name|rotation
condition|)
block|{
name|setRot
argument_list|(
name|update
operator|.
name|yaw
argument_list|,
name|update
operator|.
name|pitch
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|setRot
argument_list|(
name|yRot
argument_list|,
name|xRot
argument_list|)
expr_stmt|;
block|}
block|}
specifier|protected
name|void
name|setRot
parameter_list|(
name|float
name|pitch
parameter_list|,
name|float
name|yaw
parameter_list|)
block|{
name|yRot
operator|=
name|pitch
expr_stmt|;
name|xRot
operator|=
name|yaw
expr_stmt|;
block|}
specifier|public
name|void
name|setSize
parameter_list|(
name|float
name|width
parameter_list|,
name|float
name|height
parameter_list|)
block|{
name|bbWidth
operator|=
name|width
expr_stmt|;
name|bbHeight
operator|=
name|height
expr_stmt|;
block|}
specifier|public
name|boolean
name|shouldRender
parameter_list|(
name|Vec3D
name|vec3d
parameter_list|)
block|{
name|float
name|dx
init|=
name|x
operator|-
name|vec3d
operator|.
name|x
decl_stmt|;
name|float
name|dy
init|=
name|y
operator|-
name|vec3d
operator|.
name|y
decl_stmt|;
name|float
name|dz
init|=
name|z
operator|-
name|vec3d
operator|.
name|z
decl_stmt|;
name|dz
operator|=
name|dx
operator|*
name|dx
operator|+
name|dy
operator|*
name|dy
operator|+
name|dz
operator|*
name|dz
expr_stmt|;
return|return
name|shouldRenderAtSqrDistance
argument_list|(
name|dz
argument_list|)
return|;
block|}
specifier|public
name|boolean
name|shouldRenderAtSqrDistance
parameter_list|(
name|float
name|sqDist
parameter_list|)
block|{
name|float
name|bbSize
init|=
name|boundingBox
operator|.
name|getSize
argument_list|()
operator|*
literal|64F
decl_stmt|;
return|return
name|sqDist
operator|<
name|bbSize
operator|*
name|bbSize
return|;
block|}
specifier|public
name|void
name|tick
parameter_list|()
block|{
name|walkDistO
operator|=
name|walkDist
expr_stmt|;
name|xo
operator|=
name|x
expr_stmt|;
name|yo
operator|=
name|y
expr_stmt|;
name|zo
operator|=
name|z
expr_stmt|;
name|xRotO
operator|=
name|xRot
expr_stmt|;
name|yRotO
operator|=
name|yRot
expr_stmt|;
block|}
specifier|public
name|void
name|turn
parameter_list|(
name|float
name|pitchDiff
parameter_list|,
name|float
name|yawDiff
parameter_list|)
block|{
name|float
name|oldXRot
init|=
name|xRot
decl_stmt|;
name|float
name|oldYRot
init|=
name|yRot
decl_stmt|;
name|yRot
operator|=
operator|(
name|float
operator|)
operator|(
name|yRot
operator|+
name|pitchDiff
operator|*
literal|0.15D
operator|)
expr_stmt|;
name|xRot
operator|=
operator|(
name|float
operator|)
operator|(
name|xRot
operator|-
name|yawDiff
operator|*
literal|0.15D
operator|)
expr_stmt|;
if|if
condition|(
name|xRot
operator|<
operator|-
literal|90F
condition|)
block|{
name|xRot
operator|=
operator|-
literal|90F
expr_stmt|;
block|}
if|if
condition|(
name|xRot
operator|>
literal|90F
condition|)
block|{
name|xRot
operator|=
literal|90F
expr_stmt|;
block|}
name|xRotO
operator|+=
name|xRot
operator|-
name|oldXRot
expr_stmt|;
name|yRotO
operator|+=
name|yRot
operator|-
name|oldYRot
expr_stmt|;
block|}
block|}
end_class

end_unit

